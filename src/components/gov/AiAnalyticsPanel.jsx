...
import { Wand2, Loader2, Code, Lightbulb } from "lucide-react";

export default function AiAnalyticsPanel({ metrics }) {
            ...
            {aiResult.global_prompt && (
              <div className="p-3 rounded border bg-stone-50">
                <div className="flex items-center gap-2 font-medium mb-2">
                  <Lightbulb className="h-4 w-4" /> Actionable Prompt
                </div>
              ...
              <div key={idx} className="border rounded p-3">
                ...
                {iss.root_cause && (
                  <div className="text-stone-600 text-xs mb-2">
                    <span className="font-medium">Root cause:</span> {iss.root_cause}
                  </div>
                )}
                ...
                {iss.code_suggestion?.snippet && (
                  <div className="mt-2">
                    <div className="flex items-center gap-2 text-xs text-stone-500 mb-1">
                      <Code className="h-3 w-3" /> Suggested Code
                    </div>
                    <pre className="bg-stone-900 text-stone-100 rounded p-3 overflow-auto text-xs">
                      <code>{iss.code_suggestion.snippet}</code>
                    </pre>
                  </div>
                )}
              </div>
