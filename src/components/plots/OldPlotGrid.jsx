...
export default memo(function OldPlotGrid({ plots, isAdmin, onHover, onEdit }) {
  ...
  function plotPosition(plotNum) {
    ...
    // Col 19 (index 18): 875-920 bottom-up above N/U at row 99
    if (plotNum >= 875 && plotNum <= 920) {
      return { row: 98 - (plotNum - 875), col: 18 }; // 875 at row 98, 920 at row 53
    }
    // Col 19: blank at row 52, then 844-854 at rows 51-41
    if (plotNum >= 844 && plotNum <= 854) {
      return { row: 51 - (plotNum - 844), col: 18 }; // 844 at row 51, 854 at row 41
    }
    // Col 19: blank at row 40, then 855-861 at rows 39-33
    if (plotNum >= 855 && plotNum <= 861) {
      return { row: 39 - (plotNum - 855), col: 18 }; // 855 at row 39, 861 at row 33
    }
    // Col 19: unknown at row 32, then 862-874 at rows 31-19
    if (plotNum >= 862 && plotNum <= 874) {
      return { row: 31 - (plotNum - 862), col: 18 }; // 862 at row 31, 874 at row 19
    }
  ...
  const grid = useMemo(() => {
    ...
    rows[99][18] = { Grave: 'N/U', Status: 'Not Usable', _virtual: true }; // N/U above MOW in col 19
    rows[100][18] = { Grave: 'MOW', Status: 'Not Usable', _virtual: true }; // MOW at bottom of col 19
