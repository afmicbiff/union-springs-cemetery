...
export default function ReservePlot() {
  ...
  React.useEffect(() => {
    ...
    if (el) {
      ...
      el.classList.add('border','border-teal-200','bg-teal-50');
            ...
            {(!member.isLoading && !member.data) ? (
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2 max-h-96 overflow-auto">
                  {(filtered || []).map((r)=> (
                    <button key={r.id} data-plot-id={r.id} onClick={()=>setSelected(r)} className={`text-left border rounded p-2 hover:bg-gray-50 ${selected?.id===r.id ? 'border border-teal-200 bg-teal-50' : ''}`}>
